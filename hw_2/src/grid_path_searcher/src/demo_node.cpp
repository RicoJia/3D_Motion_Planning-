...
// receieves a path, start planning
void rcvWaypointsCallback(const nav_msgs::Path & wp)
{     
...
// update from the map realtime, then update A* obstacles
void rcvPointCloudCallBack(const sensor_msgs::PointCloud2 & pointcloud_map)
{   
    if(_has_map ) return;
...
int main(int argc, char** argv)
{
    ...
    _pts_sub  = nh.subscribe( "waypoints", 1, rcvWaypointsCallback );   //from waypoint_generator
